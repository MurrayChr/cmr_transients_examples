  ...
  matrix get_multinomial_probs(
    ...
    for (I in 1:(T-1)) {  // block row index
      for (J in I:(T-1)) {  // block column index
        // row and column indices to define the 4x4 block
        int i1 = 1 + 4*(I-1);
        int i2 = 4*I;
        int j1 = 1 + 4*(J-1);
        int j2 = 4*J;
...
data {
  ...
  array[4*(T-1), 4*(T-1)+1] int<lower=0> marr;    // m-array (see above)
...
model {
  ...
  for (t in 1:(T-1)) {
    for (s in 1:2) {
      int i = 1 + 2*(s-1) + 4*(t-1);   // indexes first (s=1) and third (s=2) rows in block t
      target += Nsingle[s,t] * log( pr[i][4*(T-1)+1] * pi_r[s,t] + (1 - pi_r[s,t]) );
    }
  }
}
